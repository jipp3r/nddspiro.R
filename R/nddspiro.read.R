...
nddspiro.read <-
  function(xmlfilename,
           indices = c("FEV1", "FVC", "FEV1_FVC", "PEFR"),
           traces = 2,
           accepted_only = TRUE,
           add_GLI = FALSE) {
    require(rspiro)
    ...
    if (!is.installed('rspiro')) {
      add_GLI <- FALSE
      cat(
        "rpsiro package is not available - unable to add GLI ranges. To install, try... devtools::install_git(\"https://github.com/thlytras/rspiro.git\")\n"
      )
    }
    ...
    for (i in 1:traces) {
      ...
      if (accepted_only)
        xmlpath <-
        paste0(xmlpath, " and Accepted='true']")
      else
        xmlpath <- paste0(xmlpath, "]")
      
      if (i == 1) {
        return.df <-
          nddspiro.results(indices,
                           xmlfile,
                           xmlpath,
        ...
        new.df <-
          nddspiro.results(indices,
                           ...
                           no_ranges)
        return.df <-
          merge(return.df, new.df, by = "nddTestID", all.x = TRUE)
      }
